...
function TeachersList() {
  ...
  const handleAddTeacher = () => {
    navigate(findPathByLabel(adminRoutes, "TeachersCreate"));
  };

  const viewAction = (id) => {
    console.log("View action triggered for ID:", id);
    navigate(findPathByLabel(adminRoutes, "TeachersDetails", { teacherId: id }));
  };

  const editAction = (id) => {
    console.log("Edit action triggered for ID:", id);
    navigate(findPathByLabel(adminRoutes, "TeachersUpdate", { teacherId: id }));
  };
  ...
  const handleDeleteEvent = async () => {
    if (!teacherIdToDelete) return; // Ensuring there's an event ID

    try {
    //   var response = await userServices.deleteUser(userIdToDelete);
       var response = await teacherServices.deleteTeacher(teacherIdToDelete);

       console.log(response);

       setData((prevData) =>
        prevData.filter((teacher) => teacher.id !== teacherIdToDelete)
       );
       ...
       enqueueSnackbar("Teacher deleted successfully!", {
        ...
        if (error.response) {
          ...
          enqueueSnackbar("Failed to delete the teacher. Please try again.", {
            variant: "error",
            autoHideDuration: 2000,
          });
        }
  ...
  return (
    <Box m="20px">
      {/* HEADER */}
      <Box mb="5px">
        <Header title="Teachers" subtitle="The list of teachers!" />
        <Button
          variant="contained"
          sx={{
